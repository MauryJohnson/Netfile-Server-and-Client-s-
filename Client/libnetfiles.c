...
//Initialize test
int E = netserverinit(HostName,0);
...
//printf("STATUS READ:%d",netread(-6,b1,500));
...
ssize_t netopen(const char* fileName, size_t intPerm){
	...
	printf("\n MessageLen:%d",Messlen);
	//if(Messlen<=1)
		//return -1;
	
	//char* errnoOrFd = (char*)malloc( sizeof(char)  *  ( Messlen ) ); //errnoOrFd now contains the errno value if the command failed
	char errnoOrFd[Messlen];
								    //or the associated fd if the command was a sucess
	//bzero(errnoOrFd,Messlen-1);
		if(errnoOrFd == NULL)
			exit(-1);
	...
	if(readMess[0] == '1' ){ 
		printf("\n 1 FAIL");
		//the front of readMess was a 1, so error, so errnoOrFd is errno. Sets errno and returns -1
		...
		printf("\n SUCCESSSSS");
		//the front of readMess was a 0, so sucess, so errnoOrFd is the fd. Now just return the fd
		free(readMess);
		SOCKFD=-1;
		MODE = "-1";
...
ssize_t netread(int fd, void* buff, size_t num){
	...
	char errnoOrBuff[Messlen];									//that was read on sucess
	...
	if(readMess[0] == '1' ){ //Message has 1 on front, thus error so errnoOrBuff is the corresponding errno value
		...
		//Message has 0 on front, thus sucess, so errnoOrBuff is the buff that was read
		free(readMess);
